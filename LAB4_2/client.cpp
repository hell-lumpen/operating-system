...
int main(int argc, char* argv[]) {
	...
	char name[128] = "", host_name[128] = "", message_rv[1024] = "", message_sd[1024] = "";

	if (argc != 5) {
		std::cout << "you need to specify arguments in the format:\n -u ip-adress:port -a nickname\n" << std::endl;
		return -1;
	}
	...
	std::cout << "nickname: " << name << "\thostname: " << host_name << "\tport: " << port << std::endl;
	...
	while(true) {
		if (pid == 0) {	
			...
			if (sendto(sock_b, message_sd, sizeof(message_sd), 0, (struct sockaddr *)&addr, size) == -1) {
				perror("sendto");
				exit(EXIT_FAILURE);
			}
		}
			...
			if (recvfrom(sock_b, message_rv, 1024, 0, (struct sockaddr *)&addr, (unsigned int*) &size) == -1) {
				perror("recvfrom");
				exit(EXIT_FAILURE);
			}
			...
			if(std::string(message_rv).substr(index) == "quit!") {
				if (sendto(sock_b, "quit!", sizeof("quit!"), 0, (struct sockaddr *)&addr, size) == -1) {
					perror("sendto");
					exit(EXIT_FAILURE);
				}
