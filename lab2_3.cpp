...
// Открыть указанный файл на запись. Если файл уже существует, сделать на него 
// жёсткую ссылку (name.x.ext), где x – первый свободный номер, ext – текущее 
// расширение файла.
...
char* magic(const char* filename, char* linkname, int index, int i, int j) {
...
int main(int argc, char const *argv[])
{
  ...
  else if (argc == 2)
  {
    ...
    if (int file = open(filename, O_WRONLY) != -1) {
      ...
      while (link(filename, linkname = magic(filename, linkname, index, i, j)) == -1)
        index++;
