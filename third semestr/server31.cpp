...
int 
Kardano(double A, double B, double C, double D, number_representation* x){
	if (A == 0) return -1;
	...
	{
	x[0].real = 
	2.0*sqrt(-p/3.0)*cos(F/3.0)-B/(3.0*A);

	x[1].real = 
	2.0*sqrt(-p/3.0)*cos((F/3.0)+2.0*M_PI/3.0)-B/(3.0*A);

	x[2].real = 
	2.0*sqrt(-p/3.0)*cos((F/3.0)+4.0*M_PI/3.0)-B/(3.0*A);
	}
	...
	{
	double temp1 = 
	mypow((-q/2.0)+sqrt(S)) + mypow((-q/2.0)-sqrt(S));
	double temp2 = 
	mypow((-q/2.0)+sqrt(S)) - mypow((-q/2.0)-sqrt(S));
	x[0].real = temp1 - B/(3.0*A);
	x[1].real = -temp1/2.0 - B/(3.0*A); 
	x[1].imaginary = sqrt(3)*temp2/2.0;
	x[2].real = -temp1/2.0 - B/(3.0*A); 
	x[2].imaginary = -sqrt(3)*temp2/2.0;
	}
	return 0;
}
...
int main () {
	...
	while (true) {
		if (msgrcv(fd, &mbuf, 1024, 1 , 0) > 0) {
			if (Kardano(mbuf.ratio[0],mbuf.ratio[1],
				mbuf.ratio[2],mbuf.ratio[3], x) == -1){
